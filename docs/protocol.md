...
Execution time per calculation or update is `kernel_ns / steps`; initialization is excluded from Figures 1–3. Total execution time includes setup, initialization and every calculation or update in the workload. Figure 4 reports total execution time for complete searches. In A, LUT conversion and state construction are also recorded separately.
