...
struct Context {
    ...
    Context(unsigned ni, unsigned mi) : n(ni), m(mi) {
        if (n < 1 || n > 20 || m < 1 || m > 20)
            throw std::runtime_error("supported dimensions: 1..20 (above 16: spectrum only)");
        length = 1U << n;
        components = 1U << m;
        words = (length + 63) / 64;
...
struct Engine {
    ...
    Engine(const Context& cx, Method me, Metric mt, const Box& s, bool prepared = false)
        ...
        if ((c.n > 16 || c.m > 16) && metric != Metric::Spectrum)
            throw std::runtime_error("above 16 bits only spectrum evaluation is supported");
