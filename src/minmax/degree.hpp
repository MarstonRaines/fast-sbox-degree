...
inline unsigned hist_max(const Hist& h) {
    for (int d = int(h.size())-1; d >= 0; --d) if (h[d]) return unsigned(d);
    throw std::runtime_error("empty spectrum");
}
...
struct Context {
    ...
    Context(unsigned ni, unsigned mi) : n(ni), m(mi) {
        if (n < 1 || n > 20 || m < 1 || m > 20)
            throw std::runtime_error("supported dimensions: 1..20");
        length = 1U << n;
        components = 1U << m;
        words = (length + 63) / 64;
        weights.resize(length);
        degree_masks.assign((n + 1) * words, 0);
        std::array<std::vector<unsigned>, 21> layers;
...
struct Engine {
    ...
    // Four 16-bit or two 32-bit output coefficient vectors per word (Algorithm 2).
    ...
    std::array<std::pair<size_t, Word>,20> top_delta{};
    ...
    Engine(const Context& cx, Method me, Metric mt, const Box& s, bool prepared = false)
        ...
        if (method == Method::Proposed && metric == Metric::Minimum) {
            output_anfs.resize(c.m <= 16 ? (c.length + 3) / 4 : (c.length + 1) / 2);
            if (packed_input) {
    ...
    void update_truth(unsigned p, unsigned q) {
        ...
        if (method == Method::Proposed && metric == Metric::Minimum) {
            if (c.m > 16) {
                output_truth[p / 2] ^= Word(delta_output) << (32 * (p % 2));
                output_truth[q / 2] ^= Word(delta_output) << (32 * (q % 2));
                return;
            }
    ...
    void pack_outputs(std::vector<Word>& dst) const {
        if (c.m > 16) {
            for (unsigned u = 0; u < c.length; u += 2)
                dst[u / 2] = Word(box[u]) | (Word(box[u+1]) << 32);
            return;
        }
        if (c.length < 4) {
    ...
    unsigned output_packed_minimum() {
        ...
        for (Word& v : output_anfs) {
            ...
            } else v ^= (v << 32) & 0xffffffff00000000ULL;
        }
        ...
        std::array<unsigned,20> basis{};
        unsigned rank = 0;
        for (unsigned u : c.order) {
            unsigned v = c.m <= 16 ? unsigned(output_anfs[u/4] >> (16*(u%4))) & 65535U
                                      : unsigned(output_anfs[u/2] >> (32*(u%2)));
            while (v) {
