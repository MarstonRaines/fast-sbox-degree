...
Input read_input(const std::string& path) {
    ...
    require(x.n >= 1 && x.n <= 16 && x.m >= 1 && x.m <= 16, "invalid dimensions");
    x.box.resize(1U << x.n);
...
template<class E> void benchmark(int argc, char** argv) {
    ...
    record << ",\"verified\":true,\"thread_limit\":" << omp_get_max_threads()
           << ",\"observed_team\":" << observed_team
           << ",\"parallel_threshold\":" << parallel_threshold()
           << ",\"worker_cpus\":";
    json_array(record, worker_cpus);
...
int main(int argc, char** argv) {
    try {
        omp_set_dynamic(0);
        worker_cpus.resize(omp_get_max_threads(), -1);
        // Warm the runtime before all dimension/state/kernel timers. External
        // process wall time still includes startup and this team creation.
        #pragma omp parallel
        {
            #pragma omp single
            observed_team = omp_get_num_threads();
            worker_cpus[omp_get_thread_num()] = sched_getcpu();
        }
        require(observed_team == omp_get_max_threads(), "runtime team smaller than requested");
