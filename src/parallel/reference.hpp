...
struct ReferenceEngine {
    ...
    ReferenceEngine(const Context& cx, Method me, Metric mt, const Box& s, bool = false)
        ...
        if (method == Method::Reference && metric != Metric::Maximum)
            scratch.assign(omp_get_max_threads(), std::vector<int32_t>(c.length));
        auto begin = std::chrono::steady_clock::now();
        parallel_for(0, c.m, uint64_t(c.m)*c.length, [&](unsigned b) {
        ...
        if (method == Method::ReferenceProposed && metric != Metric::Minimum) {
            ...
            if (metric == Metric::Maximum) {
                ...
                for (unsigned base = 1, bit = 0; base < c.components; base *= 2, ++bit) {
                    parallel_for(base, 2*base, uint64_t(base)*c.length, [&](unsigned l) {
                        auto* dst = component_anfs.data()+size_t(l)*c.length;
                        const auto* src = component_anfs.data()+size_t(l-base)*c.length;
    ...
    void make_coordinates() {
        ...
        parallel_for(0, c.m, uint64_t(c.m)*c.length*c.n, [&](unsigned b) {
    ...
    uint64_t evaluate() {
        ...
        if (method == Method::ReferenceProposed) {
            ...
            for (unsigned u : c.order) {
                ...
                parallel_for(rank+1, c.m, uint64_t(c.m-rank-1)*c.length, [&](unsigned r) {
        ...
        if (metric == Metric::Maximum) {
            std::array<unsigned,16> results{};
            parallel_for(0, c.m, uint64_t(c.m)*c.length, [&](unsigned b) {
                results[b] = full_degree(coord.data()+b*c.length);
            });
            return *std::max_element(results.begin(),results.end());
        }
        hist.fill(0);
        unsigned blocks = parallel_blocks(1, c.components, uint64_t(c.components-1)*c.length*c.m,
            [&](unsigned first, unsigned last, unsigned block) {
                ...
                for (unsigned l = first; l < last; ++l) {
                    ...
                    for (unsigned selected = l; selected; selected &= selected-1) {
                        ...
                        for (unsigned u = 0; u < c.length; ++u) buffer[u] ^= row[u];
                    }
                    unsigned d = full_degree(buffer.data());
                    if (metric == Metric::Spectrum) ++part.histogram[d];
                    else part.minimum = std::min(part.minimum,d);
                }
            });
        unsigned minimum = c.n;
        for (unsigned block = 0; block < blocks; ++block) {
            minimum = std::min(minimum, partials[block].minimum);
    ...
    uint64_t apply(unsigned p, unsigned q, bool reversible) {
        ...
        make_delta(p,q);
        if (metric == Metric::Maximum) {
            parallel_for(0, c.m, uint64_t(c.m)*delta.size(), [&](unsigned b) {
            ...
            unsigned blocks = parallel_blocks(1, c.components,
                ...
                [&](unsigned first, unsigned last, unsigned block) {
                    ...
                    for (unsigned l = first; l < last; ++l) {
                        ...
                        if (before != after) { --changes[before]; ++changes[after]; degrees[l] = after; }
                    }
                });
            for (unsigned block = 0; block < blocks; ++block)
